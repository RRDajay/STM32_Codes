...
typedef enum { eSender1,
    eSender2 } DataSource_t;
...
// Dummy Data
static Data_t xStructsToSend[2] = { { "Received from sender1", eSender1 },
...
int main_queue(void)
{
    ...
    if (xQueue != NULL) {
        ...
        xTaskCreate(vSenderTask, "Sender1", 512, &(xStructsToSend[0]),
            tskIDLE_PRIORITY + 2, NULL);
        xTaskCreate(vSenderTask, "Sender2", 512, &(xStructsToSend[1]),
            tskIDLE_PRIORITY + 2, NULL);
