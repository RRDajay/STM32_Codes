...
rcc_get_hsical(void)
{
    return RCC->CR & (0xFF << 8U);
}

// HSITRIM
uint8_t
rcc_get_hsitrim(void)
{
    return RCC->CR & (0x1F << 3U);
}
...
void rcc_tim8_clock_disable(void)
{
    RCC->APB2ENR &= ~(1U << 13U);
}
...
void rcc_iopg_clock_disable(void)
{
    RCC->APB2ENR &= ~(1U << 8U);
}
void rcc_iopf_clock_disable(void)
{
    RCC->APB2ENR &= ~(1U << 7U);
}
